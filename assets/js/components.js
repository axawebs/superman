...
function remove_subcmp_js(){
  ...
  for (let i=allsuspects.length; i>=0; i--){
      //search backwards within nodelist for matching elements to remove
    if (allsuspects[i] && allsuspects[i].getAttribute("src")!=null && (allsuspects[i].getAttribute("src").indexOf('subcmp_scripts.js')!=-1 || allsuspects[i].getAttribute("src").indexOf('subcmp_common_scripts.js')!=-1)){
        allsuspects[i].parentNode.removeChild(allsuspects[i]) //remove element by calling parentNode.removeChild()
    }
  }
}
